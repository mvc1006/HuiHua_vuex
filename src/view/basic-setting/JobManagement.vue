  ...
  <div class="content-box">
    <Card title="职级架构">
      <div class="content-body">
        <Row>
          <span>快速查询：</span>
          <Input v-model="fastQuery" placeholder="请输入职级名称" size="large" style="width: 300px"></Input>
    ...
    <Modal :mask-closable="false" v-model="showAdd" :title="title" @on-cancel="closeModal">
      <Form ref="formItem" :model="formItem" :label-width="100" :rules="formRules" class="form-box">
        <FormItem label="上级职称" prop="superiorTitle">
          <Input v-model="formItem.superiorTitle" :disabled="showTrue" placeholder="请输入上级职称"></Input>
        </FormItem>
        <FormItem
        ...
        >
          <Row>
            <Col span="18">
              <Input type="text" v-model="item.value" placeholder="请输入工作区名称"></Input>
            </Col>
            <Col span="4" offset="1">
        ...
        <FormItem :hidden="mentName">
          <Row>
            <Col span="12">
              <Button type="dashed" long @click="handleAdd" :disabled="addShow" icon="md-add">新增工作区</Button>
            </Col>
          </Row>
        </FormItem>
        <FormItem :hidden="jobName" label="工作区名称" prop="superiorTitle">
          <Input v-model="formItem.superiorTitle" :disabled="showTrue" placeholder="请输入工作区名称"></Input>
        </FormItem>
        <FormItem
          v-for="(item, index) in formItem.barCodeList"
          :hidden="jobName"
        ...
        >
          <Row>
            <Col span="18">
              <Input type="text" v-model="item.value" placeholder="请输入岗位名称"></Input>
            </Col>
            <Col span="4" offset="1">
              <Button @click="handleRemove(index)">删除</Button>
            </Col>
        ...
        <FormItem :hidden="jobName">
          <Row>
            <Col span="12">
              <Button type="dashed" long @click="handleAdd" :disabled="addShow" icon="md-add">新增岗位</Button>
            </Col>
          </Row>
        </FormItem>
      </Form>
...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        rankName: { required: true, message: "请完善信息" },
  ...
  methods: {
    ...
    // 获取每条数据
    evaluate(row) {
      this.formItem.superiorTitle = row.rankName;
      this.formItem.rankName = row.rankName;
      this.formItem.founder = row.createName;
    ...
    add(row, rowIndex) {
      ...
      console.log(this.mentName, this.jobName);
      this.formItem.superiorTitle = row.rankName;
      this.showAdd = true;
      this.newData = row;
    },
